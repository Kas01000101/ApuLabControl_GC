...
async function visibleTextCount(p,text){return p.getByText(text,{exact:true}).evaluateAll(nodes=>nodes.filter(n=>{const s=getComputedStyle(n);return s.display!=='none'&&s.visibility!=='hidden'&&n.getClientRects().length>0;}).length);}
(async()=>{
 ...
 try{
  ...
  ok(await p.locator('.control-n3-cell').count()===64,'direct: 64 cells'); ok(await visibleTextCount(p,'EXPLORAR')===0,'direct: EXPLORAR visible'); ok(await visibleTextCount(p,'AYNI')===0,'direct: AYNI visible');
